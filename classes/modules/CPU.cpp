...
/*   Updated: 2018/10/14 12:40:41 by azulbukh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			CPU::putInfoSDL( void ) const
{
	std::string		proc = saveExec("sysctl -n machdep.cpu.brand_string");
	std::string		proc1 = proc.substr(0, proc.find("(TM) ") + 5);
	std::string		proc2 = proc.substr(proc.find("(TM) ") + 5);
	std::string		architecture = saveExec("uname -p");
	std::string		ncores = saveExec("sysctl hw.ncpu | sed 's/.*: //'");
	std::string		usageUser = saveExec("top -l 1 -i 1 -n 0 | awk '/CPU usage:/ {print $3;}'");
	std::string		usageSystem = saveExec("top -l 1 -i 1 -n 0 | awk '/CPU usage:/ {print $5;}'");
	std::string		usageIdle = saveExec("top -l 1 -i 1 -n 0 | awk '/CPU usage:/ {print $7;}'");
	...
	SDL_Rect rect = {0, _frame_y, WIN_X, _nbColums * 30 + 30};
